...
import { GenericPanel, PanelComponent, TextComponent } from "maco-ui";
...
const DebugInfo = observer(props => {  
  ...
  return (
    <GenericPanel showTitle={false} panel={props.panel}>
      <PanelComponent title="graph" collapsible gutters>
        <TextComponent>
          <ul>
      ...
      <PanelComponent title="graph" collapsible gutters>
        <TextComponent>
          <pre>
            {scene.operatorGraphs &&
              JSON.stringify(getSnapshot(scene.operatorGraphs), null, 5)}
          </pre>
        </TextComponent>
      </PanelComponent>

      <PanelComponent title="target" collapsible gutters>
        <TextComponent>
      ...
      <PanelComponent title="selected node" collapsible gutters>
        <TextComponent>
          <div>
            <strong>name:</strong> {selected.name}
          </div>
          <div>
            <strong>branch:</strong> {selected.branch_index}
          </div>
          <div>
            <strong>data:</strong>{" "}
            {!selected.data ? (
          ...
          <div>
            ...
            {!selected.parents.length ? (
              ...
              <pre style={pre_style}>
                {JSON.stringify(selected.parents, null, 2)}
              </pre>
            )}
          </div>
          <div>
            ...
            {!selected.children.length ? (
              ...
              <pre style={pre_style}>
                {JSON.stringify(selected.children, null, 2)}
              </pre>
            )}
          </div>
        </TextComponent>
      ...
      <PanelComponent title="selected param" collapsible gutters>
        <TextComponent>
          ...
          <pre>
            {selectedParam &&
              JSON.stringify(getSnapshot(selectedParam), null, 5)}
          </pre>
        </TextComponent>
      </PanelComponent>
    </GenericPanel>
